# coding: utf-8

"""
    Argo Workflows API

    Argo Workflows is an open source container-native workflow engine for orchestrating parallel jobs on Kubernetes. For more information, please see https://argoproj.github.io/argo-workflows/  # noqa: E501

    The version of the OpenAPI document: VERSION
    Generated by: https://openapi-generator.tech
"""

import re  # noqa: F401
import sys  # noqa: F401
import typing  # noqa: F401
import functools  # noqa: F401

from frozendict import frozendict  # noqa: F401

import decimal  # noqa: F401
from datetime import date, datetime  # noqa: F401
from frozendict import frozendict  # noqa: F401

from argo_workflows.schemas import (  # noqa: F401
    AnyTypeSchema,
    ComposedSchema,
    DictSchema,
    ListSchema,
    StrSchema,
    IntSchema,
    Int32Schema,
    Int64Schema,
    Float32Schema,
    Float64Schema,
    NumberSchema,
    UUIDSchema,
    DateSchema,
    DateTimeSchema,
    DecimalSchema,
    BoolSchema,
    BinarySchema,
    NoneSchema,
    none_type,
    Configuration,
    Unset,
    unset,
    ComposedBase,
    ListBase,
    DictBase,
    NoneBase,
    StrBase,
    IntBase,
    Int32Base,
    Int64Base,
    Float32Base,
    Float64Base,
    NumberBase,
    UUIDBase,
    DateBase,
    DateTimeBase,
    BoolBase,
    BinaryBase,
    Schema,
    _SchemaValidator,
    _SchemaTypeChecker,
    _SchemaEnumMaker
)


class GrpcGatewayRuntimeStreamError(
    DictSchema
):
    """NOTE: This class is auto generated by OpenAPI Generator.
    Ref: https://openapi-generator.tech

    Do not edit the class manually.
    """
    
    
    class details(
        ListSchema
    ):
    
        @classmethod
        @property
        def _items(cls) -> typing.Type['GoogleProtobufAny']:
            return GoogleProtobufAny
    grpc_code = IntSchema
    http_code = IntSchema
    http_status = StrSchema
    message = StrSchema


    def __new__(
        cls,
        *args: typing.Union[dict, frozendict, ],
        details: typing.Union[details, Unset] = unset,
        grpc_code: typing.Union[grpc_code, Unset] = unset,
        http_code: typing.Union[http_code, Unset] = unset,
        http_status: typing.Union[http_status, Unset] = unset,
        message: typing.Union[message, Unset] = unset,
        _configuration: typing.Optional[Configuration] = None,
        **kwargs: typing.Type[Schema],
    ) -> 'GrpcGatewayRuntimeStreamError':
        return super().__new__(
            cls,
            *args,
            details=details,
            grpc_code=grpc_code,
            http_code=http_code,
            http_status=http_status,
            message=message,
            _configuration=_configuration,
            **kwargs,
        )

from argo_workflows.model.google_protobuf_any import GoogleProtobufAny
